...
# ==== CREATE THE PLOT ====
p <- ggplot(plot_data, aes(x = behavior_label, y = n, fill = class)) +
  ...
  labs(
    title = "Vehicle Behavior by Spacing Type",
    subtitle = "Stacked bars showing vehicle class distribution across departure and brake behaviors",
    x = "Behavior",
    y = "Count"
  ) +
...
# Save plot as SVG
output_dir <- "/Users/benjaminhailstone/Library/CloudStorage/Box-Box/2024-tprs/output"
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

cat("\nSaving bar chart as SVG...\n")
chart_path <- file.path(output_dir, "stacked_bar_chart.svg")
ggsave(chart_path, plot = p, width = 18, height = 8, units = "in", device = "svg")
cat("Bar chart saved to:", chart_path, "\n")
